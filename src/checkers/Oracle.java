...
public class Oracle {
    ...
    public double evaluateBoard2(Board board, Player player) {
        double boardValue = 0;
        int scoreMode = 0;
        if((board.isWhiteWinner() && player == Player.white) || (board.isBlackWinner() && player == Player.black))
            return POINT_WON;
        else if((board.isWhiteWinner() && player == Player.black) || (board.isBlackWinner() && player == Player.white))
            return -POINT_WON;
        ...
        for(int i = 0; i < Board.rows; i++){
            for (int j = 0; j < Board.cols; j++) {
                if(board.cell[i][j].equals(CellEntry.empty) || board.cell[i][j].equals(CellEntry.black) || board.cell[i][j].equals(CellEntry.blackKing))
                    continue;
                else
                    countPieces++;
                for (int k = 0; k < Board.rows; k++) {
                    for (int l = 0; l < Board.cols; l++) {
                        if(board.cell[i][j].equals(CellEntry.empty) || board.cell[i][j].equals(CellEntry.white) || board.cell[i][j].equals(CellEntry.whiteKing))
                            continue;
                        double dx = Math.abs(j - l);
                        double dy = Math.abs(i - k);
        ...
        distance /= countPieces;
        boardValue = 1/distance * scoreMode;
        return boardValue;
    }
...
//            wValue += BoardPositionPoints(board);
            wValue /= (board.blackKings + board.blackPieces);
        }
...
//            bValue += BoardPositionPoints(board);
            bValue /= (board.whiteKings+board.whitePieces);
        }

        return bValue;
    }

    private int countPieces(Board board, CellEntry check){
        int coun = 0;
        for(int i = 0; i< 8; i++)
            for(int j = 0; j < 8; j++)
                if(board.cell[i][j].equals(check))
                    coun++;
        return  coun;
